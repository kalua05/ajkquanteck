...
const AssetManager = {
  async upload(file, meta) { return new Promise((resolve)=>{ const reader=new FileReader(); reader.onload=()=>{ const asset={id:Date.now(), name:file.name, url:reader.result, ...meta}; const assets=DB.get('assets')||[]; assets.push(asset); DB.set('assets',assets); resolve({ok:true, asset}); }; reader.readAsDataURL(file); }); },
